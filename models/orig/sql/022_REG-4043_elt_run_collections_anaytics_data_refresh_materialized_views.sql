...
left outer join salesforce_rss.sf_case_history    b
on a.id = b.caseid
...
inner join (
    ...
    group by case_casenumber,case_id
)  coltask
on cccd.col_id = coltask.mincolidAND cccd.case_id = coltask.case_id;
...
from pro_sandbox.ca_salesforce_stage1 cssd
    left outer join  
...
select a.match_key
 ...
 from pro_sandbox.ca_salesforce_driver1   a
 inner join pro_sandbox.ca_payments1            b
...
select a.match_key
      ...
      from pro_sandbox.ca_ptp_unique1 a
            inner join pro_sandbox.ca_payments1   b
          on a.match_key = b.match_key
...
update pro_sandbox.ca_sf_task_penetration_calls_stage1
    set create_date = (select max(b.task_create_dttm) from pro_sandbox.ca_salesforce1 b where a.taskid = b.taskid)
...
,taskid
,Nbr_of_Calls
from pro_sandbox.ca_sf_task_penetration_calls_stage1;
...
create table pro_sandbox.ca_ColHistoryTaskDt1 as
  ...
  from pro_sandbox.ca_salesforce1 s
...
from pro_sandbox.ca_salesforce1 a
...
where  cast(motr.partition_0 as date) BETWEEN cast(a.case_created_dttm as date) AND cast(coalesce(a.case_closed_dttm,getdate()) as date) 
group by a.case_id,a.match_key;
...
from pro_sandbox.ca_VARHistory1 vh
    left outer join collections_history_prod_rss.mckinsey_value_at_risk_naf mnaf
...
select distinct 
    ...
    ,null::varchar(100) as dialer_contact_method
from pro_sandbox.ca_salesforce1 a -- materialized view
left outer join pro_sandbox.ca_get_amt_due1 b -- materialized view
on a.case_casenumber = b.case_casenumber and a.case_create_dt  = b.case_create_dt
left outer join pro_sandbox.ca_Promise_Payments1 e
on a.ptpid = e.ptpid
left outer join pro_sandbox.ca_sf_payments1 f
on a.case_casenumber = f.case_casenumber and a.match_key       = f.match_key  and a.case_create_dt  = f.case_create_dt 
left outer join naflt_psfin_rss.ps_customer      g
on a.match_key = g.cust_id
left outer join naflt_dw_owner_rdw_rss.dw_acct   h
on a.match_key = h.wex_acct_nbr  
left outer join pro_sandbox.ca_efs_customer_current1 i
on a.match_key = i.cust_id
left outer join pro_sandbox.ca_nice_acd1                         j
on a.task_cxone_contact_id   = j.nice_contact_id and length(a.task_cxone_contact_id)>2 
left outer join pro_sandbox.ca_sf_task_penetration_calls1           k
on a.case_id = k.case_id AND a.taskid = k.taskid AND a.task_create_dttm = k.create_date AND k.taskid IS NOT NULL
 left outer join pro_sandbox.ca_TaskLevelActionFlagC  tla
  on a.taskid = tla.taskid AND a.task_create_dttm = tla.task_create_dttm AND a.col_id = tla.hcolid
    left join pro_sandbox.ca_otr_platform1 op
  on a.match_key = op.ar_number
     left outer join pro_sandbox.ca_VarInfobyCase1 vic
  on a.case_id = vic.case_id 
    ...
    left outer join pro_sandbox.ca_collection_queue_hist1 ccqhd
...
inner join (
    ...
    group by 
        ca.contact_id ,cast(ca.contact_start as date),s.outbound_strategy
) ds
...
select 
    ...
    '                                                   ' as promise_kept
    ,null::float as ttl_paid_to_promise
from salesforce_rss.sf_promise_to_pay sptp 
...
group by sptp.id,sa.id,sptp.createddate,sptpc.casenumber,cc.q_type,cc.lob,coalesce(NULLIF(sa.wex_account__c,''),NULLIF(sc.ar_number__c,'')),sptp.collections__c,sup.name;
...
select a.match_key
      ...
      from pro_sandbox.ca_ptp_all1 a
	        left outer join pro_sandbox.ca_payments1  b
		  on a.match_key = b.match_key
...
from pro_sandbox.ca_ptp_all1 ptp_all
